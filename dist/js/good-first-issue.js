...
async function fetchRepoIssues(repo) {
  ...
  issues = issues.filter((i) =>
    (i?.labels || []).find((l) => l.name === "good first issue")
  );
  if (issues.length === 0) {
    return;
  }
  ...
  for (const issue of issues) {
    ...
    small.textContent = `#${issue.number} opened on ${fixDate(
      issue.created_at
    )} by ${issue.user.login}`;
    ...
    issueCommentsLink.className =
      issue.comments > 0 ? "list-item-actions show" : "d-none";
...
async function loadGithubIssues() {
  try {
    ...
    for (const pendingIssues of promises) {
      const { repoIssues, count, repo } = (await pendingIssues) || {};
